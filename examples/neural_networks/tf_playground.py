...
def visualize_decision_boundary_gradient(model, X, Y, show_training_data=True):
...
def visualize_decision_boundary_points(model, X, Y, point_spacing = .5, show_training_data=True):
    # Generate a grid of points from -5 to 5 with a step of 0.1
    ...
    # Plot the decision boundary
    plt.scatter(xx, yy, c=zz, cmap=colormaps['RdBu'], alpha=0.8)
    if show_training_data:
        plt.scatter(X[Y == 0][:, 0], X[Y == 0][:, 1], c='red', marker='o', label='Class 0')
        plt.scatter(X[Y == 1][:, 0], X[Y == 1][:, 1], c='blue', marker='s', label='Class 1')
        plt.xlabel('Feature 1')
...
for epoch in range(num_epochs):
    ...
    if epoch % math.ceil(num_epochs/(30-1)) == 0:  # max of 30 plots, 29 while training and 1 after
        visualize_decision_boundary_points(net, dataset.points, dataset.labels)
...
visualize_decision_boundary_points(net, dataset.points, dataset.labels,  show_training_data=False)
